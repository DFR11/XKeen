...
- `port_proxying.lst` - proxy ports, for example 80 and 443
- `port_exclude.lst` - ports that need to be excluded from proxying, for example, 3389.
- `ip_exclude.lst` - IP addresses and subnets that need to be excluded from proxying, for example, 77.88.8.8

Each port and IP is indicated in these files on a new line. Empty lines and lines starting with a comment character `#` are ignored. Proxy ports and ports excluded from proxying cannot be used together, use one or the other. If both lists are filled by mistake, the proxy ports have priority and the list of excluded ports is ignored. During a clean installation of XKeen, templates of the above files are created with examples of filling them out.
...
## Self-Hosted proxy for downloading
...
If GitHub is unavailable, updating geofiles using the scheduler will not work; perform it manually.
