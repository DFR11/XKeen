...
choice_geoip() {
    ...
    while true; do
        ...
        echo 
        echo -e "Select the action number or numbers separated by a space for ${yellow}GeoIP${reset}"
        echo 

        [ "$has_missing_geoip_bases" = true ] && echo "1. Install missing ones and update installed GeoIPs" || echo -e "1. ${italic}All available GeoIPs are installed${reset}"
