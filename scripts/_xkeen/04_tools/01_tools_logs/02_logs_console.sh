...
logs_delete_geosite_info_console() {
    ...
    if [ -f "$geo_dir/geosite_zkeen.dat" ]; then
        ...
        info_content="${info_content} ${green}Success${reset}: File geosite_zkeen.dat is missing from directory'$geo_dir'\n"
    fi
...
logs_delete_register_yq_info_console() {
    ...
    if [ ! -f "$register_dir/yq_s.list" ]; then
        info_content="${green}Success${reset}: File yq_s.list not found in directory'$register_dir/'"
    else
        error_content="${red}Error${reset}: File yq_s.list found in directory'$register_dir/'"
    fi

    if [ ! -f "$register_dir/yq_s.control" ]; then
        info_content="${info_content}\n ${green}Success${reset}: File yq_s.control not found in directory'$register_dir/'"
    else
        error_content="${error_content}\n ${red}Error${reset}: File yq_s.control found in directory'$register_dir/'"
    fi

    if ! grep -q 'Package: yq_s' "$status_file"; then
        info_content="${info_content}\n ${green}Success${reset}: Package yq registration not found in'$status_file'"
    else
        error_content="${error_content}\n ${red}Error${reset}: Package yq registration detected in'$status_file'"
    fi
