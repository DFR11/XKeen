# Function to get the version from the xkeen API and store it in a variable
info_version_xkeen() {
    version=$(curl -m 10 -s "$xkeen_api_url" | jq -r '.tag_name // .name // ""' 2>/dev/null)

    if [ -z "$version" ]; then
        echo
        printf "${red}No access${reset} to ${yellow}GitHub API${reset}, try ${yellow}jsDelivr${reset}...\n"
